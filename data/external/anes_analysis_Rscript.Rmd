...
security.var <- c("v201249", "v201429", "v201432x", "v201311x", "v202310", "v202232", 
                  ...
                  "v202351", "v202332")
security.names <- c("defense.spending", "urban.unrest", "protestors.actions", 
...
all_party_cor <- function(x, y, z){
  ...
  df <- as.data.frame(matrix(NA, length(x), 8))
  for(i in 1:nrow(df)){
    variable <- x[i]
    anes1[, variable][anes1[, variable] < 1 | anes1[, variable] == 99] <- NA
    variance <- wtd.var(anes1[, variable], weight = anes1$v200010a)[1] 
  ...
  colnames(df) <- c("Issue", "ANES_Variable","Cor_to_Party_ID", "Normalized_Variance", 
                    "Variance", "Point_Scale", "Type", "Source")
  df
}
...
cor_var_mat <- ggplot(all.df, aes(Cor_to_Party_ID, Normalized_Variance, label = Issue)) +
  ...
  geom_text_repel(aes(color = Type), max.overlaps = 11, size = 1.7) +
  ...
  annotate("text", x = 0.035, y = .36, label = "Mean Variance", size = 2.5, fontface = 2) +
  annotate("text", x = .432, y = .07, label = "Mean Correlation", size = 2.5, fontface = 2)
